 ...
 */
package cn.odboy.modules.mnt.service.impl;
...
import cn.odboy.modules.mnt.domain.App;
import cn.odboy.modules.mnt.domain.Deploy;
import cn.odboy.modules.mnt.domain.DeployHistory;
import cn.odboy.modules.mnt.domain.Server;
import cn.odboy.modules.mnt.mapper.DeployMapper;
import cn.odboy.modules.mnt.mapper.DeployServerMapper;
import cn.odboy.modules.mnt.service.DeployHistoryService;
import cn.odboy.modules.mnt.service.DeployService;
import cn.odboy.modules.mnt.service.ServerService;
import cn.odboy.modules.mnt.domain.vo.DeployQueryCriteria;
import cn.odboy.modules.mnt.util.ExecuteShellUtil;
import cn.odboy.modules.mnt.util.ScpClientUtil;
import cn.odboy.infra.websocket.constant.MsgTypeEnum;
import cn.odboy.infra.websocket.model.SocketMsg;
import cn.odboy.infra.websocket.core.WebSocketServer;
...
public class DeployServiceImpl extends ServiceImpl<DeployMapper, Deploy> implements DeployService {
	...
	private void deployApp(String fileSavePath, Long id) {
		...
		if (deploy == null) {
			sendMsg("部署信息不存在", MsgTypeEnum.ERROR);
			throw new BadRequestException("部署信息不存在");
		}
		App app = deploy.getApp();
		if (app == null) {
			sendMsg("包对应应用信息不存在", MsgTypeEnum.ERROR);
	...
	private void sendMsg(String msg, MsgTypeEnum msgType) {
		try {
	...
	public String serverStatus(Deploy resources) {
		...
		for (Server server : servers) {
			...
			if (result) {
				sb.append("<br>正在运行");
				sendMsg(sb.toString(), MsgTypeEnum.INFO);
			} else {
				sb.append("<br>已停止!");
				sendMsg(sb.toString(), MsgTypeEnum.ERROR);
			}
	...
	public String startServer(Deploy resources) {
		...
		for (Server deploy : deploys) {
			...
			sendMsg("下发启动命令", MsgTypeEnum.INFO);
			executeShellUtil.execute(app.getStartScript());
			sleep(3);
			sendMsg("应用启动中，请耐心等待启动结果，或者稍后手动查看运行状态", MsgTypeEnum.INFO);
			int i  = 0;
			boolean result = false;
			// 由于启动应用需要时间，所以需要循环获取状态，如果超过30次，则认为是启动失败
			while (i++ < count){
	...
	public String stopServer(Deploy resources) {
		...
		for (Server deploy : deploys) {
			...
			ExecuteShellUtil executeShellUtil = getExecuteShellUtil(deploy.getIp());
			sb.append("服务器:").append(deploy.getName()).append("<br>应用:").append(app.getName());
			sendMsg("下发停止命令", MsgTypeEnum.INFO);
			//停止应用
			stopApp(app.getPort(), executeShellUtil);
			sleep(1);
			boolean result = checkIsRunningStatus(app.getPort(), executeShellUtil);
	...
	public String serverReduction(DeployHistory resources) {
		...
		if (app == null) {
			sendMsg("应用信息不存在：" + resources.getAppName(), MsgTypeEnum.ERROR);
		...
		stopApp(app.getPort(), executeShellUtil);
		//删除原来应用
		sendMsg("删除应用", MsgTypeEnum.INFO);
		executeShellUtil.execute("rm -rf " + deployPath + FILE_SEPARATOR + resources.getAppName());
		//还原应用
		sendMsg("还原应用", MsgTypeEnum.INFO);
		executeShellUtil.execute("cp -r " + backupPath + "/. " + deployPath);
		sendMsg("启动应用", MsgTypeEnum.INFO);
		executeShellUtil.execute(app.getStartScript());
		sendMsg("应用启动中，请耐心等待启动结果，或者稍后手动查看启动状态", MsgTypeEnum.INFO);
		int i  = 0;
		boolean result = false;
		// 由于启动应用需要时间，所以需要循环获取状态，如果超过30次，则认为是启动失败
		while (i++ < count){
	...
	private ExecuteShellUtil getExecuteShellUtil(String ip) {
		...
		if (server == null) {
			sendMsg("IP对应服务器信息不存在：" + ip, MsgTypeEnum.ERROR);
